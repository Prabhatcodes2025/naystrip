...
import { clean, dateOnly, money, normalizeEmail, validEmail, uuidPattern } from "../_validation.js";
...
async function notifyB2BLead(record,partnerEmail){const payload={reference:record.id,kind:record.kind,customerName:record.name,phone:record.phone,email:record.email,destination:record.destination};const jobs=[];if(process.env.LEADS_NOTIFICATION_EMAIL)jobs.push(deliverNotification({inquiryId:record.id,event:"new_enquiry_internal",recipient:process.env.LEADS_NOTIFICATION_EMAIL,payload,idempotencyKey:`b2b-lead-admin:${record.id}`}));if(partnerEmail)jobs.push(deliverNotification({inquiryId:record.id,event:"enquiry_acknowledged",recipient:partnerEmail,payload,idempotencyKey:`b2b-lead-partner:${record.id}`}));const results=await Promise.allSettled(jobs);for(const result of results)if(result.status==="rejected"||result.value?.status==="failed")console.error("b2b_lead_notification_failed",{inquiryId:record.id,status:result.status});}
...
export default async function handler(req,res){
 ...
 try{
  if(req.method==="GET"){
   ...
   const catalogue=packages.map((pkg)=>{const configured=rateByPackage.get(pkg.id),publicPrice=amount(pkg.price_from),netRate=amount(configured?.agent_price),configuredMarkup=amount(configured?.markup),markupPercent=Math.min(100,configuredMarkup??defaultMarkupPercent);return {...pkg,rateId:configured?.id||null,publicPrice,netRate,markupPercent,sellingPrice:netRate===null?null:money(netRate*(1+markupPercent/100)),validFrom:configured?.valid_from||null,validUntil:configured?.valid_until||null,rateSource:configured?"configured_b2b":"not_configured"}});
  ...
  if(body.action==="create_enquiry"){
   ...
   const customerEmail=normalizeEmail(body.customerEmail);if(customerEmail&&!validEmail(customerEmail))return json(res,422,{error:"Enter a valid customer email address"});
   ...
   const requirements=Array.isArray(body.requirements)?body.requirements.map((item)=>clean(item,50)).filter(Boolean):[];const record={id:reference("NAYE"),kind:"b2b_enquiry",agent_id:agentId,package_id:packageId,name:clean(body.customerName,120),phone:clean(body.customerPhone,24),email:customerEmail||null,destination,status:"new",source:packageId?"B2B Package Enquiry":"B2B General Enquiry",enquiry_source:packageId?"package":"general",payload:{agency:session.profile.business_name,travelDate,pax,adults,children,rooms:Math.max(1,Math.min(50,Number(body.rooms||1))),requirements,notes:clean(body.notes,3000),packageTitle:pkg?.title||null,packageDuration:pkg?`${pkg.nights}N/${pkg.days}D`:null}};
   const insert=await supabaseRequest("inquiries",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify(record)});if(!insert.ok)return json(res,502,{error:"Enquiry could not be saved"});const created=(await insert.json())[0];await notifyB2BLead(created||record,session.profile.email);return json(res,201,{enquiry:created})
  }
  if(body.action==="create_quotation"){
   const quotationEmail=normalizeEmail(body.customerEmail);if(quotationEmail&&!validEmail(quotationEmail))return json(res,422,{error:"Enter a valid customer email address"});body.customerEmail=quotationEmail;
