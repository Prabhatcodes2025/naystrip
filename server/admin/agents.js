...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    const response = await supabaseRequest("b2b_agents?select=id,user_id,business_name,contact_person,email,phone,gst_number,pan,address,website,business_type,verification_status,created_at,updated_at,documents:agent_documents(id,document_type,object_path,verified_at)&order=created_at.desc&limit=500");
    return response.ok ? json(res, 200, { agents: await response.json() }) : json(res, 502, { error: "Partner applications could not be loaded" });
  }
