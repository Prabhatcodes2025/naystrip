...
export default async function handler(req,res){const admin=await requireAdmin(req,res);if(!admin)return;
 try{
  if(req.method==="GET"){const status=clean(req.query?.status,30);const kind=clean(req.query?.kind||"custom_trip",30);const select=kind==="b2b_enquiry"?"*,agent:b2b_agents(id,business_name),package:packages(id,title,slug)":"*";const query=`inquiries?kind=eq.${encodeURIComponent(kind)}${status?`&status=eq.${encodeURIComponent(status)}`:""}&select=${select}&order=created_at.desc&limit=500`;const response=await supabaseRequest(query);const rows=await response.json();if(!response.ok)return json(res,502,{error:"Unable to load leads"});return json(res,200,{leads:rows})}
