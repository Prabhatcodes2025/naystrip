...
export async function applySuccessfulPayment(payment,entity,actor="cashfree"){
  ...
  const payload={reference:payment.booking.reference,customerName:payment.booking.billing?.name,packageTitle:payment.booking.package?.title,travelDate:payment.booking.travel_date,amountPaid,balanceDue:balance,portalUrl:`${(process.env.PUBLIC_SITE_URL||"https://www.naystrip.com").replace(/\/$/,"")}/account/dashboard`};
