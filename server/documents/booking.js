...
export default async function handler(req,res){
  ...
  try{
    ...
    if(documentId){
      const response=await supabaseRequest(`booking_documents?id=eq.${documentId}&select=*,booking:bookings(reference,customer_id),invoice:invoices(invoice_number,booking_id)&limit=1`);const documents=await response.json();
      ...
      const document=documents[0];if(!document)return json(res,404,{error:"Document not found"});if(!admin&&document.booking?.customer_id!==user.id)return json(res,403,{error:"You do not have access to this document"});
