...
const dataTable = (headers, data, widths) => {const right=headers.map(value=>/^(qty|quantity|rate|unit price|amount|total)$/i.test(value));return new Table({ width: { size: 9360, type: WidthType.DXA }, columnWidths: widths, rows: [new TableRow({ tableHeader: true, children: headers.map((header, index) => cell(header, { bold: true, fill: FOREST, color: WHITE, width: widths[index], alignment:right[index]?AlignmentType.RIGHT:undefined })) }), ...data.map((values) => new TableRow({ children: values.map((value, index) => cell(value, { fill: PALE, width: widths[index], alignment:right[index]?AlignmentType.RIGHT:undefined })) }))] });};
const keyValueTable = (rows) => dataTable(["DETAIL", "INFORMATION"], rows.map((item) => [item.label, item.value]), [2700, 6660]);
const compactMetaTable=(rows)=>{const base=Math.floor(9360/rows.length),widths=rows.map((_,index)=>index===rows.length-1?9360-base*(rows.length-1):base);return new Table({width:{size:9360,type:WidthType.DXA},columnWidths:widths,rows:[new TableRow({children:rows.map((item,index)=>new TableCell({width:{size:widths[index],type:WidthType.DXA},borders,shading:{type:ShadingType.CLEAR,fill:WHITE},margins:{top:90,bottom:90,left:120,right:120},children:[paragraph(String(item.label).toUpperCase(),{bold:true,color:ORANGE,size:14,after:30}),paragraph(item.value,{bold:true,size:18,after:0})]}))})]})};
