...
test("booking CTA state requires real owner-controlled configuration",()=>{
  ...
  assert.equal(calculateBookingState(base,[{start_date:future,status:"open",available_seats:5,price_override:null}]).online,true);
  assert.equal(calculateBookingState({...base,policies:{booking_mode:"fixed_departure"}},[]).code,"departure_required");
  assert.equal(calculateBookingState({...base,price_from:null,policies:{booking_mode:"fixed_departure"}},[{start_date:future,status:"open",available_seats:5,price_override:14000}]).online,true);
});
