...
const heroSlides = [
  { tour: tours[0], kicker: "Mumbai & Maharashtra", title: "Stories begin where the usual route ends.", copy: "Thoughtfully paced city, coast, heritage and hill journeys, planned by a team based in Navi Mumbai." },
  { tour: tours[7], kicker: "Konkan Coast", title: "Follow the coast at your own rhythm.", copy: "Build a route around beaches, food, forts and stays that suit the people travelling with you." },
  { tour: tours[5], kicker: "Western Ghats", title: "Trade the checklist for a better journey.", copy: "Flexible itineraries, transparent inclusions and practical on-trip support from first call to return." },
  { tour: tours[1], kicker: "Heritage circuits", title: "Go deeper than a weekend away.", copy: "Connect Maharashtra’s remarkable caves, temples and historic cities without rushing between them." },
];
...
export default function Home() {
  ...
  return <>
    ...
    <section className="relative min-h-[760px] overflow-hidden bg-[#102f29] text-white">
      ...
      <div className="container-lg relative flex min-h-[760px] items-center pb-28 pt-24">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-5 max-w-4xl font-display text-5xl leading-[.96] sm:text-7xl lg:text-[84px]">{active.title}</h1>
    ...
    <section className="overflow-hidden bg-[#173c34] py-20 text-white sm:py-28"><div className="container-lg"><div className="max-w-3xl"><p className="eyebrow !text-orange-300">Choose your landscape</p><h2 className="mt-4 font-display text-4xl leading-tight sm:text-6xl">Places worth slowing down for.</h2></div><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{destinations.slice(0, 4).map((destination) => <Link key={destination.slug} to={`/destinations/${destination.slug}`} className="group relative min-h-[420px] overflow-hidden rounded-[1.5rem]"><img src={destination.image} alt={destination.name} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" loading="lazy"/><div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/5 to-transparent"/><div className="absolute inset-x-0 bottom-0 p-6"><p className="text-xs font-bold uppercase tracking-[.16em] text-orange-300">{destination.bestTime}</p><h3 className="mt-2 font-display text-3xl">{destination.name}</h3><p className="mt-2 text-sm text-white/70">{destination.tagline}</p></div></Link>)}</div></div></section>
