...
import { Eye, Trash2, X, Search, CheckCircle2 } from "lucide-react";
import { getContactLeads, updateContactLeadStatus, deleteContactLead } from "../../utils/storage";
...
export default function AdminContactLeads() {
  ...
  const refresh = () => setLeads(getContactLeads());
  const handleStatusChange = (id, status) => { updateContactLeadStatus(id, status); refresh(); };
  const handleDelete = (id) => { if (confirm("Delete this submission?")) { deleteContactLead(id); refresh(); setViewing(null); } };
  const markContacted = (id) => { updateContactLeadStatus(id, "Contacted"); refresh(); };
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-semibold text-navy-900">Contact Leads</h1>
      <p className="text-sm text-navy-500 mt-1 mb-6">Submissions from the Contact page and inquiry forms across the site.</p>
      ...
      <div className="rounded-2xl bg-white border border-navy-100 shadow-soft overflow-x-auto">
        <table className="w-full text-sm min-w-[780px]">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} className="p-8 text-center text-navy-400">No submissions yet.</td></tr>
            ) : filtered.map((l) => (
              <tr key={l.id} className="border-b border-navy-50 last:border-0">
                ...
                <td className="p-4 text-navy-500">{l.phone || l.email}</td>
                <td className="p-4 text-navy-500">{l.destination || "—"}</td>
                ...
                <td className="p-4">
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => markContacted(l.id)} title="Mark as contacted" className="flex h-8 w-8 items-center justify-center rounded-lg bg-forest-50 text-forest-600 hover:bg-forest-100"><CheckCircle2 size={14} /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-lg rounded-2xl bg-white p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-xl font-semibold text-navy-900">Submission Details</h3>
              <button onClick={() => setViewing(null)}><X size={20} /></button>
            </div>
            <dl className="space-y-3 text-sm">
              {Object.entries({
                "Reference ID": viewing.id, Name: viewing.name, Phone: viewing.phone, Email: viewing.email,
                Destination: viewing.destination || "—", Source: viewing.source || "Contact Page",
                Message: viewing.message || "—", Status: viewing.status,
              }).map(([k, v]) => (
