...
const columns={
 ...
 Services:[["International holidays","/holidays"],["Custom holidays","/custom-trip"],["Group & family trips","/custom-trip"],["School & college tours","/custom-trip"],["Transport","/transport"],["Visa assistance","/custom-trip?service=visa"],["Travel insurance","/custom-trip?service=insurance"],["Hotel assistance","/custom-trip?service=hotels"]],
 Support:[["FAQs","/faqs"],["Terms & conditions","/terms-and-conditions"],["User agreement","/user-agreement"],["Privacy policy","/privacy-policy"],["Cancellation policy","/cancellation-policy"],["Refund policy","/refund-policy"],["Payment policy","/payment-policy"],["Sitemap","/sitemap"]],
 Portal:[["Customer login","/account/login"],["Customer registration","/account/register"],["B2B partner","/b2b/login"],["B2B registration","/b2b/register"],["Admin","/admin/login"]],
};
...
export default function Footer(){
 ...
 return <footer className="bg-[#102f29] text-white/65"><div className="container-lg py-16 sm:py-20"><div className="grid gap-12 lg:grid-cols-[1.3fr_2fr]"><div><Link to="/" aria-label="NaysTrip home" className="inline-flex"><BrandLogo className="h-24 w-auto"/></Link><p className="mt-5 max-w-sm text-sm leading-7">{s.footerText}</p><div className="mt-6 flex gap-3">{socials.filter(([key])=>Boolean(s.social?.[key])).map(([key,Icon,label])=><a key={key} className="grid h-10 w-10 place-items-center border border-white/15 hover:text-orange-300" href={s.social[key]} target="_blank" rel="noopener noreferrer" aria-label={label}><Icon size={17}/></a>)}</div></div><div className="grid grid-cols-2 gap-8 sm:grid-cols-4">{Object.entries(columns).map(([title,links])=><div key={title}><h2 className="text-xs font-extrabold uppercase tracking-[.16em] text-white">{title}</h2><ul className="mt-5 space-y-3 text-sm">{links.map(([label,to])=><li key={`${label}-${to}`}><Link to={to} className="hover:text-orange-300">{label}</Link></li>)}</ul></div>)}</div></div>
